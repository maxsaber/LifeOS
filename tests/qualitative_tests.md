...
**Query:** "What are my recent interactions with Ben Calvin?"

**Orchestration Steps:**
1. **People** - Resolve "Ben Calvin" to entity, get:
   - Email(s): e.g., ben@murmuration.org
   - Company: Murmuration
   - Role: (if known)
   - Category: work
...
4. **Vault** - Search in `Work/Murm/` folder for mentions of "Ben" or "Calvin" (since he's at Murmuration)
5. **iMessage** - Skip or deprioritize (work contact, unlikely to text)

**Expected Searches:**
| Source | Query/Filter | Time Range |
|--------|-------------|------------|
| People | name match "Ben Calvin" | Entity resolution |
| Gmail | from:ben@murmuration.org OR to:ben@murmuration.org | Last 60 days |
| Calendar | attendees contains ben@murmuration.org | Last 60 days + next 30 days |
| Vault | path:Work/Murm/* AND (Ben OR Calvin) | Last 90 days, high relevance |
...
- Context: "Ben is [role] at Murmuration"

#### Report Card: Test 2.1 (Evaluated 2026-01-09)
| Criterion | Pass? | Notes |
|-----------|-------|-------|
...
| Got company context (Murmuration) | ✅ | |
...
| Targeted Vault search to Work/Murm/ folder | ☐ | Not yet implemented - searches all vault |
...
| Source | Query/Filter | Time Range |
|--------|-------------|------------|
| Calendar | date = 2026-01-03 | Exact date |
| Vault | filename: "2026-01-03" OR "January 3" | Exact date |
| Gmail | after:2026/01/03 before:2026/01/04 | Exact date |
| iMessage | date = 2026-01-03 | Exact date |
...
### 2026-01-09 - Initial Systematic Testing

**Issues Found & Fixed:**

1. **Person interaction queries routing too narrow**
   - Issue: Queries like "recent interactions with Ben Calvin" only routed to `['people']`
...
**Remaining Work:**
- Vault folder targeting based on entity context (Work/Murm/ for Murmuration contacts)
